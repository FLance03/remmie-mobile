//importing 'screens.dart' imports all screens located
//in the user_screens folder

export 'user_screens/LoginPage.dart';
export 'user_screens/SignupPage.dart';
export 'user_screens/HomePage.dart';
export 'user_screens/HotelsPage.dart';
export 'user_screens/BookingPage1.dart';
export 'user_screens/BookingPage2.dart';
export 'user_screens/BookingPage3.dart';
export 'user_screens/RoomServicePage2.dart';
export 'user_screens/RoomServicePage.dart';
export 'user_screens/RoomServicePage3.dart';
export 'user_screens/HotelDetailsPage.dart';

